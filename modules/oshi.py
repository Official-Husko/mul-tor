...
class Oshi:
    ...
    def Uploader(file, proxy_list, user_agents):
        try:
            ...
            if calc_size == "OK":
                if proxy_list == []:
                    req = requests.post(url=upload_url, files=files_data, headers={"User-Agent": ua})
                else:
                    req = requests.post(url=upload_url, files=files_data, headers={"User-Agent": ua}, proxies=random.choice(proxy_list))
